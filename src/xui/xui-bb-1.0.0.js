    ...
    if (re.test(xhtml)) {
        ...
        if (result[2] !== "") {
            ...
            for (; i < attrList.length; i++) {
                ...
                if (attr !== "" && attr !== " ") {
                    node = attr.split('=');
                    attributes[cleanString(node[0])] = cleanString(node[1]); //.replace(/([\"|\']?)/g, '');
                }
            }
        }
        xhtml = result[3];
...
"inner outer top bottom remove before after".split(' ').forEach(function (method) {
  xui.fn[method] = function(action) { return function (html) { return this.html(method, html); };}(method);
});
